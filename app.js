...
async function loadProducts() {
  ...
  // Prevent stale cart entries from crashing the cart when a product was deleted.
...
function updateCart() {
  ...
  $('#cartItems').innerHTML = items.length
    ...
    : '<div class="cart-empty">Кошик порожній. Додайте перші товари.</div>';

  const total = items.reduce((s, { i, p }) => s + p.price * i.qty, 0);
  $('#cartTotal').textContent = fmt(total);

  document.querySelectorAll('[data-inc]').forEach(b =>
    b.onclick = () => change(Number(b.dataset.inc), 1)
  );
  document.querySelectorAll('[data-dec]').forEach(b =>
    b.onclick = () => change(Number(b.dataset.dec), -1)
  );
